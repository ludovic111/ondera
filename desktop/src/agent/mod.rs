...
Musical conventions: bars and beats are zero-based; note start and length are beats relative to their clip; pitch 60 is C4; velocity 1-127; 0.75 is unity gain on faders. Write whole patterns with clip_create or clip_setNotes in one call. Keep notes inside their clip. Song sections are markers on the ruler (marker_list, marker_add with a name such as Verse 1 or Chorus, marker_goto, marker_cycleSection to loop one). Audio clips take fades in seconds and a gain in dB (clip_setFades, clip_setGain). Use the stock instruments from session_catalog and factory presets from preset_list when they fit.\n\
