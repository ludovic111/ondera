  ...
  const onContextMenu = (e: MouseEvent<HTMLDivElement>) => {
    ...
    const colors: MenuEntry[] = Object.entries(TRACK_PALETTE).map(
      ([name, color]) => ({
        ...
        onSelect: () =>
          dispatch(commands.track.setColor({ trackId: track.id, color })),
      }),
    );
    setMenu({
      x: e.clientX,
      ...
      items: [
        ...
        separator,
        {
          ...
          onSelect: () =>
            dispatch(
              commands.track.move({ trackId: track.id, index: index - 1 }),
            ),
        },
        {
          label: "Move Down",
    ...
    >
      ...
      <div className={styles.controls} onClick={(e) => e.stopPropagation()}>
        ...
        <HSlider
          ...
          onChange={(v) =>
            dispatch(commands.track.setVolume({ trackId: track.id, volume: v }))
          }
        />
        <Knob
