  ...
  return (
    <div className={styles.bar} data-surface="transport">
      <div className={styles.group}>
        <Button
          title="Return"
          onClick={() => dispatch(commands.transport.returnToStart({}))}
        >
          <ReturnIcon />
        </Button>
        <Button
          title="Rewind"
          onClick={() => dispatch(commands.transport.nudge({ bars: -1 }))}
        >
          <RewindIcon />
        </Button>
        <Button
      ...
      <div className={styles.group}>
        ...
        <Button
          title="Stop · twice to return to start"
          onClick={() => dispatch(commands.transport.stop({}))}
        >
          <StopIcon />
        </Button>
        <Button
        ...
        <Button
          title="Cycle (C) · drag in the ruler to set the range"
          pressed={cycle}
          onClick={() =>
            dispatch(commands.transport.setCycle({ enabled: !cycle }))
          }
        >
          <CycleIcon />
        </Button>
      ...
      <div className={styles.group}>
        <Button
          ...
          pressed={metronome}
          onClick={() =>
            dispatch(commands.transport.setMetronome({ enabled: !metronome }))
          }
        >
          Click
        </Button>
      ...
      <div className={styles.right}>
        <div className={styles.meter}>
          ...
          <div className={`${styles.masterWell} m-well-meter`}>
            <LedStrip
              ...
              level={Math.max(
                0,
                (20 * Math.log10(Math.max(1e-6, meters.masterL)) + 60) / 66,
              )}
              hot={2}
            />
            <LedStrip
              ...
              level={Math.max(
                0,
                (20 * Math.log10(Math.max(1e-6, meters.masterR)) + 60) / 66,
              )}
              hot={2}
            />
          </div>
          <div className={styles.readout}>
            {Math.max(meters.masterL, meters.masterR) > 0
              ? formatDb(masterDb, 1)
              : "−∞"}
          </div>
        </div>
        ...
        <Button
          ...
          onClick={() =>
            dispatch(commands.view.setAgentPanelOpen({ open: !agentOpen }))
          }
        >
