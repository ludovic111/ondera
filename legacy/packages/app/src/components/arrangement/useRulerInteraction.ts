...
export function useRulerInteraction() {
  ...
  const localX = (e: ReactPointerEvent<HTMLElement>) => e.clientX - e.currentTarget.getBoundingClientRect().left;
  ...
  const onPointerDown = useCallback(
    (e: ReactPointerEvent<HTMLDivElement>) => {
      ...
      if (t.cycle) {
        ...
        if (Math.abs(x - x0) <= size.cycleGrip) drag.current = { kind: 'edge', edge: 'start', startBar: t.cycleStartBar, endBar: t.cycleEndBar };
        else if (Math.abs(x - x1) <= size.cycleGrip) drag.current = { kind: 'edge', edge: 'end', startBar: t.cycleStartBar, endBar: t.cycleEndBar };
      }
      if (!drag.current) {
        drag.current = { kind: 'pending', startX: x, anchorBar: Math.max(0, Math.round(xToBar(x, geo))) };
      }
  ...
  const onPointerMove = useCallback(
    (e: ReactPointerEvent<HTMLDivElement>) => {
      ...
      if (!d) {
        const t = state.transport;
        const nearEdge = t.cycle && (Math.abs(x - barToX(t.cycleStartBar, geo)) <= size.cycleGrip || Math.abs(x - barToX(t.cycleEndBar, geo)) <= size.cycleGrip);
      ...
      if (d.kind === 'pending') {
        ...
        cur = { kind: 'range', anchorBar: d.anchorBar, startBar: Math.min(d.anchorBar, bar), endBar: Math.max(d.anchorBar, bar) };
        drag.current = cur;
      }
      if (cur.kind === 'range') {
  ...
  const onPointerUp = useCallback(
    (e: ReactPointerEvent<HTMLDivElement>) => {
      ...
      if (d.kind === 'pending') {
        ...
        const bar = e.altKey ? raw : snapBars(raw, state.transport.snapDivision, state.transport.timeSignature);
        store.dispatch(commands.transport.setPosition({ beats: barsToBeats(bar, state.transport.timeSignature) }));
        return;
      }
      store.dispatch(commands.transport.setCycleRange({ startBar: d.startBar, endBar: d.endBar }));
      if (!state.transport.cycle) store.dispatch(commands.transport.setCycle({ enabled: true }));
    },
    [store],
  );
