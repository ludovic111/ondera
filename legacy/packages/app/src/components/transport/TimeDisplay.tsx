...
import { beatsToBarBeat, beatsToSeconds, commands, formatSmpte, secondsToSmpte } from '@ondera/core';
...
const SIGNATURES: [number, number][] = [[4, 4], [3, 4], [6, 8], [5, 4], [7, 8], [2, 4], [12, 8]];
...
export function TimeDisplay() {
  ...
  const onTempoMove = (e: ReactPointerEvent<HTMLDivElement>) => {
    if (!tempoDrag.current || !e.currentTarget.hasPointerCapture(e.pointerId)) return;
    const delta = (tempoDrag.current.y - e.clientY) / TEMPO_DRAG_PX;
    const next = Math.round(Math.min(400, Math.max(20, tempoDrag.current.tempo + (e.shiftKey ? delta / 10 : delta))) * 10) / 10;
  ...
  const openSigMenu = (e: MouseEvent<HTMLElement>) => {
    ...
    setMenu({
      ...
      items: SIGNATURES.map(([n, d]) => ({
        ...
        onSelect: () => dispatch(commands.transport.setTimeSignature({ numerator: n, denominator: d })),
      })),
    });
  };
    ...
    setMenu({
      ...
      items: ['maj', 'min'].flatMap((mode) =>
        KEYS.map((k) => ({
          label: `${k} ${mode}`,
          checked: key === `${k} ${mode}`,
          onSelect: () => dispatch(commands.transport.setKey({ key: `${k} ${mode}` })),
        })),
      ),
    });
  };
  ...
  return (
    <div className={`${styles.display} m-well-deep`}>
        ...
        {editingTempo ? (
          <InlineEdit
            ...
            onCommit={(text) => {
              ...
              if (Number.isFinite(bpm)) dispatch(commands.transport.setTempo({ bpm: Math.min(400, Math.max(20, bpm)) }));
      ...
      <div className={`${styles.cell} ${styles.editable}`} onClick={openSigMenu} title="Time signature">
        <div className={styles.label}>Sig</div>
        <div className={styles.value}>
          {sig.numerator}/{sig.denominator}
        </div>
      </div>
      <div className={`${styles.cell} ${styles.last} ${styles.editable}`} onClick={openKeyMenu} title="Key">
