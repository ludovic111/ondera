...
export function drawRuler(ctx: CanvasRenderingContext2D, w: number, h: number, state: Session): void {
  ...
  for (let bar = firstBar; bar <= lastBar; bar++) {
    ...
    if (geo.ppb >= 24) {
      ...
      for (let b = 1; b < 4; b++) {
        ctx.fillRect(Math.round(x + (b * geo.ppb) / 4), h - timeline.rulerTickH, 1, timeline.rulerTickH);
      }
    }
  }
  ...
  const label = formatBarBeatShort(beatsToBarBeat(transport.positionBeats, transport.timeSignature));
  ctx.font = monoFont('value');
