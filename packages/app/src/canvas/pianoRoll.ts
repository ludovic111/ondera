import { beatsPerBar, beatsToBars, type Clip, type Note, type Session } from '@ondera/core';
import { canvasShadow, color, fill, line, radius, size, timeline, white } from '../theme/tokens';
import { withLightness } from '../theme/color';
import { cc, hline, monoFont, roundRectPath, uiFont, withShadows } from './paint';
...
export function editorLowPitch(clip: Clip | null): number {
  if (!clip || clip.data.kind !== 'midi' || clip.data.notes.length === 0) return 48;
  let lo = 127;
  for (const n of clip.data.notes) if (n.pitch < lo) lo = n.pitch;
  return Math.max(0, Math.min(127 - size.keyRows + 1, Math.floor(lo / 12) * 12));
}
...
export function rollGeometry(state: Session, width: number): RollGeometry {
  ...
  const ppb = Math.max(timeline.editorMinPxPerBar, Math.min(timeline.editorPxPerBar, Math.floor(width / bars)));
  return {
    clip,
    ppb,
    pxPerBeat: ppb / bpb,
    rowH: size.keyRow,
    rows: size.keyRows,
    low: editorLowPitch(clip),
    gridTop: size.editorRuler,
    bpb,
  };
}

export const pitchAtY = (geo: RollGeometry, y: number) => geo.low + (geo.rows - 1 - Math.floor((y - geo.gridTop) / geo.rowH));
export const beatAtX = (geo: RollGeometry, x: number) => x / geo.pxPerBeat;
export const yOfPitch = (geo: RollGeometry, pitch: number) => geo.gridTop + (geo.rows - 1 - (pitch - geo.low)) * geo.rowH;

/** Note under (x, y) and whether the cursor is on its right edge. */
export function hitTestNote(geo: RollGeometry, x: number, y: number): { note: Note; edge: boolean } | null {
  if (!geo.clip || geo.clip.data.kind !== 'midi' || y < geo.gridTop) return null;
  const pitch = pitchAtY(geo, y);
  const beat = beatAtX(geo, x);
  const notes = geo.clip.data.notes;
  for (let i = notes.length - 1; i >= 0; i--) {
    const n = notes[i]!;
    if (n.pitch !== pitch || beat < n.start || beat >= n.start + n.length) continue;
    const endX = (n.start + n.length) * geo.pxPerBeat;
    return { note: n, edge: endX - x <= size.noteEdgeGrip && n.length * geo.pxPerBeat > size.noteEdgeGrip * 2 };
  }
  return null;
}
...
export function drawPianoRoll(ctx: CanvasRenderingContext2D, w: number, h: number, state: Session, overlay: RollOverlay = {}): void {
  ...
  for (let b = 0; b < bars; b++) {
    ...
    if (mode === 'step' && stepBeats * pxPerBeat >= 6) {
      ...
      for (let k = stepBeats; k < bpb; k += stepBeats) ctx.fillRect(Math.round(x + k * pxPerBeat), gridTop, 1, h - gridTop);
    }
  }
  ...
  if (clip && clip.data.kind === 'midi') {
    ...
    for (const n of clip.data.notes) {
      ...
      const nw = Math.max(3, Math.round((mode === 'step' ? Math.max(n.length, stepBeats) : n.length) * pxPerBeat) - 2);
      const y = gridTop + r * rowH + 0.5;
      const nh = rowH - 1;
      withShadows(ctx, selected ? canvasShadow.noteSelected : canvasShadow.note, () => {
...
function drawScore(ctx: CanvasRenderingContext2D, w: number, h: number, state: Session, geo: RollGeometry): void {
  ...
  const yOfStep = (step: number) => staffTop + gap * 4 - ((step - bottomLineStep) * gap) / 2;

  ctx.fillStyle = cc(line.staff);
  for (let i = 0; i < 5; i++) ctx.fillRect(0, Math.round(staffTop + i * gap), w, 1);
  ...
  ctx.fillText(`${state.transport.timeSignature.numerator}/${state.transport.timeSignature.denominator} · ${state.transport.key} · treble`, 6, gridTop + 6);
  ...
  for (const n of clip.data.notes) {
    ...
    ctx.fillStyle = cc(line.staff);
    if (step < bottomLineStep) for (let s = bottomLineStep - 2; s >= step; s -= 2) ctx.fillRect(x - 7, Math.round(yOfStep(s)), 14, 1);
    if (step > bottomLineStep + 8) for (let s = bottomLineStep + 10; s <= step; s += 2) ctx.fillRect(x - 7, Math.round(yOfStep(s)), 14, 1);
    ctx.fillStyle = n.agent ? cc(color.accent) : cc(line.noteHead);
    ctx.beginPath();
    ctx.ellipse(x, y, timeline.noteHeadR + 1, timeline.noteHeadR, -0.35, 0, Math.PI * 2);
    ctx.fill();
    // Stem, up for low notes and down for high ones.
    const stemUp = step < bottomLineStep + 4;
    ctx.fillRect(stemUp ? x + timeline.noteHeadR : x - timeline.noteHeadR - 1, stemUp ? y - gap * 3 : y, 1, gap * 3);
    if (sharp) {
      ctx.font = monoFont('small');
    ...
    if (n.length > 1) {
      ctx.fillStyle = cc(line.staff);
      ctx.fillRect(x + timeline.noteHeadR + 2, Math.round(y), Math.round((n.length - 1) * pxPerBeat) - 4, 1);
    }
  }
}
