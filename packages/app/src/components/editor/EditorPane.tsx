...
type Drag =
  | { kind: 'move'; noteId: string; grabBeat: number; origStart: number; origPitch: number; start: number; pitch: number; length: number; startX: number; startY: number; moved: boolean }
  | { kind: 'resize'; noteId: string; start: number; pitch: number; length: number; origLength: number }
  | { kind: 'pencil'; anchor: number; start: number; length: number; pitch: number };
...
export function EditorPane() {
  ...
  const selectedNote = useSession((s) => {
    ...
    return c?.data.kind === 'midi' ? (c.data.notes.find((n) => n.id === s.view.selectedNoteId) ?? null) : null;
  });
  ...
  const canvasRef = useCanvasSurface(useCallback((ctx, w, h) => drawPianoRoll(ctx, w, h, store.getState(), overlay), [store, overlay]));
  ...
  const snapB = (b: number, fine: boolean) => (fine ? b : snapBeats(b, store.getState().transport.snapDivision));

  const audition = (pitch: number, vel = velocity) => {
    if (track) engine.previewNote(track.id, pitch, vel);
  };
  ...
  const onPointerDown = (e: ReactPointerEvent<HTMLDivElement>) => {
    ...
    if (hit) {
      ...
      if (mode === 'step') {
        dispatch(commands.note.remove({ clipId: clip.id, noteId: hit.note.id }));
        return;
      }
      drag.current = hit.edge
        ? { kind: 'resize', noteId: hit.note.id, start: hit.note.start, pitch: hit.note.pitch, length: hit.note.length, origLength: hit.note.length }
        : {
            kind: 'move',
            noteId: hit.note.id,
  ...
  const onPointerMove = (e: ReactPointerEvent<HTMLDivElement>) => {
    ...
    if (!d) {
      const hit = clip && p.y >= geo.gridTop ? hitTestNote(geo, p.x, p.y) : null;
      e.currentTarget.style.cursor = hit ? (hit.edge ? 'ew-resize' : 'default') : mode === 'step' ? 'cell' : 'crosshair';
      return;
    }
    if (!clip) return;
    const lengthBeats = clip.lengthBars * geo.bpb;
    ...
    if (d.kind === 'move') {
      ...
      d.moved = true;
      d.start = Math.max(0, Math.min(lengthBeats - d.length, snapB(beat - d.grabBeat, e.altKey)));
      const pitch = Math.max(0, Math.min(127, pitchAtY(geo, p.y)));
      if (pitch !== d.pitch) {
        d.pitch = pitch;
        audition(pitch);
      }
      setOverlay({ ghost: { start: d.start, length: d.length, pitch: d.pitch } });
    } else if (d.kind === 'resize') {
      d.length = Math.max(minLen, Math.min(lengthBeats - d.start, snapB(beat, e.altKey) - d.start));
      setOverlay({ ghost: { start: d.start, length: d.length, pitch: d.pitch } });
    } else {
      ...
      setOverlay({ pencil: { start: d.start, length: d.length, pitch: d.pitch } });
    }
  };
  ...
  const onPointerUp = () => {
    ...
    if (d.kind === 'move') {
      if (d.moved && (d.start !== d.origStart || d.pitch !== d.origPitch)) {
        dispatch(commands.note.update({ clipId: clip.id, noteId: d.noteId, start: d.start, pitch: d.pitch }));
      }
    } else if (d.kind === 'resize') {
    ...
    setMenu({
      ...
      items: [
        ...VELOCITIES.map((v) => ({
          ...
          onSelect: () => dispatch(commands.note.update({ clipId: clip.id, noteId: hit.note.id, velocity: v })),
        })),
        { separator: true as const },
        { label: 'Delete Note', shortcut: '⌫', onSelect: () => dispatch(commands.note.remove({ clipId: clip.id, noteId: hit.note.id })) },
      ],
    });
  };
  ...
  const openVelocityMenu = (e: MouseEvent<HTMLElement>) => {
    ...
    setMenu({
      ...
      items: VELOCITIES.map((v) => ({
        ...
        onSelect: () => {
          setVelocity(v);
          if (clip && selectedNote) dispatch(commands.note.update({ clipId: clip.id, noteId: selectedNote.id, velocity: v }));
        },
      })),
    });
  };
    ...
    <div className={styles.pane}>
      <div className={styles.header}>
        ...
        {clip && track ? (
          ...
          <div className={styles.range}>Select a MIDI clip to edit it, or draw one with the pencil tool</div>
        )}
        <div className={styles.params}>
          <span>
            <span className={styles.dim}>Quantize</span> 1/{snap}
          </span>
          <Button size="auto" className={styles.paramButton} onClick={openVelocityMenu} title="Velocity for new notes">
            <span className={styles.dim}>Velocity</span>&nbsp;{selectedNote ? selectedNote.velocity : velocity}
          </Button>
          <span>
            <span className={styles.dim}>Scale</span> {key.replace('min', 'minor').replace('maj', 'major')}
          </span>
        </div>
      </div>
