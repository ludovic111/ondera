...
import { commands, ZOOM_MAX_PX_PER_BAR, ZOOM_MIN_PX_PER_BAR, type ArrangeTool } from '@ondera/core';
import { useDispatch, useSession } from '../../state/session';
import { SegmentedControl } from '../primitives/SegmentedControl';
import { GridIcon, PencilIcon, PointerIcon, ScissorsIcon } from '../primitives/Icons';
...
const zoomToT = (ppb: number) => (Math.log(ppb) - LOG_MIN) / (LOG_MAX - LOG_MIN);
...
export function ArrangementToolbar() {
  ...
  const zoomFromEvent = (e: PointerEvent) => {
    ...
    const t = Math.min(1, Math.max(0, (e.clientX - rect.left - size.zoomThumb / 2) / travel));
    return tToZoom(t);
  };
  ...
  return (
    <div className={styles.bar}>
      <SegmentedControl items={TOOLS} value={tool} icons onChange={(id) => dispatch(commands.view.setArrangeTool({ tool: id }))} />
      <div className={styles.info}>
        <span>
          <span className={styles.dim}>Grid</span> Bar
        </span>
        <span>
          <span className={styles.dim}>Cycle</span> {cycleStart + 1} – {cycleEnd + 1}
        </span>
      </div>
      <div className={styles.zoom}>
        <span>Zoom</span>
        <div ref={railRef} className={`${styles.rail} m-groove-alt`} onPointerDown={onPointerDown} onPointerMove={onPointerMove}>
          <div className={styles.thumb} style={{ left: `calc(${zoomToT(ppb)} * (100% - var(--size-zoom-thumb)))` }} />
        </div>
      </div>
    </div>
  );
